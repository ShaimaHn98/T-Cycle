    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mAuth.getCurrentUser().getUid() != null)
        {
            firestore.collection("User_Info").document(mAuth.getCurrentUser().getUid())
                ...
                public void onSuccess(DocumentSnapshot documentSnapshot) {
                    ...
                    {
                        imguri = documentSnapshot.get("img_profile").toString();
                        Glide.with(getBaseContext()).load(documentSnapshot.get("img_profile").toString()).into(img_profile_upadte);
                        txt_phone_num.setText(documentSnapshot.get("phone_num").toString());
                        txt_username.setText(documentSnapshot.get("username").toString());
                        if (documentSnapshot.get("gender").toString().equals("Female"))
                            r_f.setChecked(true);
                        else
                            r_m.setChecked(true);
        ...
        btn_update.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (lat == null)
                {
                    Toast.makeText(getBaseContext(),"الرجاء إدخال الموقع",Toast.LENGTH_LONG).show();
                    return;
                }
                    ...
                    firestore.collection("User_Info").document(uid).set(user_info).addOnCompleteListener(new OnCompleteListener<Void>() {
                        ...
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()) {
                                ...
                                Toast.makeText(getApplicationContext(), "تم تحديث بياناتك الشخصية بنجاح", Toast.LENGTH_LONG).show();
